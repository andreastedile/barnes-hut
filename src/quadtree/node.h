...
class Node {
 public:
   ...
   **/
  [[nodiscard]] Eigen::Vector2d center_of_mass() const;
   ...
   * box
   */
  Node(const Eigen::Vector2d &bottom_left, const Eigen::Vector2d &top_right);

  Node(const Eigen::Vector2d &bottom_left, const Eigen::Vector2d &top_right, Fork fork);

  Node(const Eigen::Vector2d &bottom_left, const Eigen::Vector2d &top_right, Leaf leaf);

  /**
   * Inserts a new body in the quadtree. The body must be located inside of the node's bounding box.
   * @details Depending on the node's current type, the insertion of the new body produces a different effect:
   * <ul>
   * <li> Empty leaf: the leaf will contain the new body;
   * <li> Leaf with a body:
   * <ul>
   * <li> If the new body coincides with the body in the leaf,
   * the mass of the body in the leaf increases by the mass of the new body;
   * <li> Otherwise, the node's type becomes Fork (i.e., a node with four children);
   * the body in the leaf is relocated in a child, and the new body is recursively inserted in a child node.
   * </ul>
   * <li> Fork: the new body is recursively inserted in one of the node's children.
   * </ul>
   * The choice of which child node to select when inserting a body depends on the body's subquadrant.
   * All the operations have the effect of changing the node's center of mass and total mass.
   * @param new_body to insert
   * @throw invalid_argument if the body for which the insertion is attempted is located outside of the node's bounding box
   */
   ...
   */
  Subquadrant get_subquadrant(const Eigen::Vector2d &point);

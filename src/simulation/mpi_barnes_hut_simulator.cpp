...
AlignedBox2d compute_bounding_box_for_processor(const AlignedBox2d& bbox, int n_procs, int proc_id) {
  ...
  double min_x = bbox.min().x() + (proc_id % N_ROWS) * (bbox.sizes().x() / N_ROWS);
  double max_x = min_x + (bbox.sizes().x() / N_ROWS);

  double min_y = bbox.min().y() + std::floor(proc_id / N_COLS) * (bbox.sizes().y() / N_COLS);
  double max_y = min_y + (bbox.sizes().y() / N_COLS);
